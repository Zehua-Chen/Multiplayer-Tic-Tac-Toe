...
export class Game {
  ...
  public getMovingPlayerName(req: Request, res: Response) {
    ...
    if (this.playerWhoJustMoved == this.hostPlayer)
      movingPlayer = this.guestPlayer;
  ...
  public createGame(req: Request, res: Response) {
    ...
    if (!this.board) {
      ...
      response.message =
        "A game has already been created. The server can only host one game";
    }
      ...
      if (body.invitationCode == this.invitationCode) {
        ...
        if (this.hostPlayer && this.hostPlayer.name == body.name) {
          ...
          if (body.name != this.hostPlayer!.name) {
            ...
            this.logPost(
              "/join_game",
              `new player joined = ${this.guestPlayer.name}`
            );
            ...
            this.logPost(
              "/join_game",
              "name conflict caused by the new player"
            );
          }
        } else {
          ...
          this.logPost(
            "'/join_game",
            "already have two player, cannot take any more;"
          );
  ...
  public move(data: any) {
    ...
    const { name, invitationCode, password } = moveData;
    ...
    if (invitationCode == this.invitationCode) {
      ...
      if (!this.playerWhoJustMoved || this.playerWhoJustMoved.name != name) {
        ...
        this.logWebsocketReceive(
          "move",
          `${moveData.name} at (y: ${y}, x: ${x}`
        );
        ...
        if (movingPlayer && movingPlayer.name != "") {
          ...
          this.logWebsocketEmit(
            "update_progress",
            `new moving = ${movingPlayer!.name}`
          );
        }
